...
def format_date(
        ...
        target_date_format: str) -> DataFrame:
    return apply_terminal_operation(
        df, field, lambda c, t: __format_date(
            c, current_date_format, target_date_format))
...
def format_date_with_predicate(df: DataFrame,
    ...
    return apply_terminal_operation_with_predicate(
        df,
        field,
        lambda c,
        t: __format_date(
...
def __format_date(
        primitive_column: Column,
        current_date_format: str,
        target_date_format: str) -> Column:
    """
    ...
    """
    date_as_ts = F.to_timestamp(primitive_column.cast(
        StringType()), current_date_format)
    return F.when(
        F.isnull(primitive_column),
        primitive_column.cast(
            StringType())) .otherwise(
        (F.when(
            F.isnull(date_as_ts),
            F.raise_error(
                f"Wrong pattern {current_date_format}. "
                f"Could not convert provided value to timestamp.")) .otherwise(
            F.date_format(
                date_as_ts,
                target_date_format).cast(
                StringType()))))
