...
def rename_with_strategy(
        df: DataFrame,
        rename_func: FieldRenameFunc) -> DataFrame:
...
class FieldRenameProcessor(CoreProcessor):
    ...
    def process(self, df: DataFrame) -> DataFrame:
        ...
        updated_schema = schema_renamer.rename_fields(
            df.schema, self.rename_func)
...
class SchemaRenamer:
    ...
    def rename_fields(
            ...
            field_rename_func: FieldRenameFunc) -> StructType:
        data_type = copy(dataTypeArg)
        if isinstance(data_type, StructType):
            data_type.fields = [self.__update_field_name(
                f, field_rename_func) for f in data_type.fields]
        elif isinstance(data_type, ArrayType):
            data_type.elementType = self.rename_fields(
                data_type.elementType, field_rename_func)
        return data_type

    def __update_field_name(
            self,
            field: StructField,
            rename_func: FieldRenameFunc) -> StructField:
        field = copy(field)
