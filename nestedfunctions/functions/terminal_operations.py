...
def apply_terminal_operation(df: DataFrame, field: str, f: Callable[[
                             Column, AtomicType], Column]) -> DataFrame:
...
def apply_terminal_operation_with_predicate(df: DataFrame,
    ...
    return LambdaBasedTerminalOperationPredicate(
        field, f, predicate_key, predicate_value).process(df)


class LambdaBasedTerminalOperation(TerminalOperationProcessor):
    def __init__(self, column: str, f: Callable[[
                 Column, AtomicType], Column]) -> None:
...
class LambdaBasedTerminalOperationPredicate(
