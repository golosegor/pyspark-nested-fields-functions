...
class DropProcessor(AnyLevelCoreProcessor):
    ...
    @staticmethod
    def is_field_or_ancestor_in_fields_to_drop(
            field: str, fields_to_drop: Set[str]) -> bool:
    ...
    @classmethod
    def consolidate_fields_to_drop(
            cls,
            dataframe_schema: StructType,
            fields_to_drop: List[str]) -> List[str]:
        fields_to_drop = set(fields_to_drop)
        non_existent_fields = {
            field for field in fields_to_drop if not cls.schema_util.does_column_exist(
                dataframe_schema, field)}
        if non_existent_fields:
            log.warning(
                f"Column(s) {non_existent_fields} don't exist. Ignoring")
        fields_to_drop -= non_existent_fields

        flattened_fields = cls.schema_util.flatten_schema(dataframe_schema)
        fields_to_keep = {
            x for x in flattened_fields if not cls.is_field_or_ancestor_in_fields_to_drop(
                x, fields_to_drop)}
        ...
        # By sorting the parents make sure to start from the root and drop the
        # parent as early as possible.
        parents_of_fields_to_drop = {
            parent for field in fields_to_drop for parent in cls.schema_util.parents_for_field(field)}
        ...
        while index < len(parents_of_fields_to_drop):
            ...
            child_fields_to_keep = {
                x for x in fields_to_keep if x.startswith(f"{parent_field}.")}
            if not child_fields_to_keep:
                # Remove all the children of parent_field from fields_to_drop
                # and add parent_field
                fields_to_drop.add(parent_field)
                fields_to_drop -= {
                    x for x in fields_to_drop if x.startswith(f"{parent_field}.")}

                # Remove all the children of parent_field from parents_of_fields_to_drop to avoid unnecessary iterations.
                # Removing these children in combination with increasing the
                # index is safe as parents_of_fields_to_drop is sorted.
                parents_of_fields_to_drop = [
                    x for x in parents_of_fields_to_drop if not x.startswith(f"{parent_field}.")]
            index += 1
        return list(fields_to_drop)

    def process(self, df: DataFrame) -> DataFrame:
        log.debug(f"Got {self.fields_to_drop} as fields_to_drop")
        self.fields_to_drop = self.consolidate_fields_to_drop(
            df.schema, self.fields_to_drop)
        log.debug(
            f"Consolidated these fields_to_drop to {self.fields_to_drop}")
        ...
        # The direct parents will be sorted by decreasing depth as
        # fields_to_drop is sorted by decreasing depth.
        direct_parent_of_fields_to_drop = {}
        for field_to_drop in self.fields_to_drop:
            direct_parent, field = self.schema_util.parent_child_elements(
                field_to_drop, raise_exception_if_no_parent=False)
            direct_parent = direct_parent if direct_parent else "ROOT"
            fields_for_direct_parent = direct_parent_of_fields_to_drop.get(
                direct_parent, [])
        ...
        # While walking the tree for the deepest fields_to_drop will already
        # drop the fields that share any ancestor.
        while self.direct_parent_of_fields_to_drop:
            deepest_remaining_parent, children_of_deepest_remaining_parent = list(
                self.direct_parent_of_fields_to_drop.items())[0]
    ...
    def _process_field_with(self,
        ...
        # terminal operation reached. Two use-cases. Primitive array -> normal
        # field
        if next is None:
            return self.apply_terminal_operation_on_structure(
                schema, current_column, current_column_name, previous)

        # Already drop what can be dropped while walking the tree !
        col_name, _ = self.schema_util.parent_child_elements(previous)
        fields_to_drop_for_current_column = self.direct_parent_of_fields_to_drop.get(
            col_name)
        if fields_to_drop_for_current_column:
            current_column = current_column.dropFields(
                *fields_to_drop_for_current_column)
            # Do the cleanup
            self.fields_to_drop = [
                x for x in self.fields_to_drop if x not in fields_to_drop_for_current_column]
            del self.direct_parent_of_fields_to_drop[col_name]

        return current_column.withField(
            f'`{current_column_name}`',
            self._process_field_recursive(
                schema=schema,
    ...
    def apply_terminal_operation_on_structure(self,
        ...
        # but can also be a transform / lambda combination applied to this
        # parent.

        col_name, _ = self.schema_util.parent_child_elements(previous)
        column = column.dropFields(
            *self.direct_parent_of_fields_to_drop[col_name])
        # Do the cleanup
        self.fields_to_drop = [
            x for x in self.fields_to_drop if x not in self.direct_parent_of_fields_to_drop[col_name]]
        del self.direct_parent_of_fields_to_drop[col_name]
        return column

    def apply_terminal_operation_on_root_level(
            self, df: DataFrame, column_name: str) -> DataFrame:
        df = df.drop(*self.direct_parent_of_fields_to_drop["ROOT"])
        # Do the cleanup
        self.fields_to_drop = [
            x for x in self.fields_to_drop if x not in self.direct_parent_of_fields_to_drop["ROOT"]]
