...
def truncate(df: DataFrame, field: str, character_size: int) -> DataFrame:
    return apply_terminal_operation(
        df, field, lambda c, t: __truncate(
            c, character_size))
...
def __truncate(primitive_value: Column, character_size: int) -> Column:
    if character_size < 0:
        ...
        # but this does not work because "column is not callable". I didn't
        # find a way so this workaround is here
        max_size = 9999_9999
        return F.reverse(
            F.substring(
                F.reverse(primitive_value), -character_size + 1, max_size))
    else:
