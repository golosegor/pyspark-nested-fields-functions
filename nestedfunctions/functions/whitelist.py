...
class WhitelistProcessor(CoreProcessor):
    ...
    def process(self, df: DataFrame) -> DataFrame:
        if self.no_fields_to_select(df, self.whitelist_fields):
            log.warning(
                f"No fields to select {self.whitelist_fields}. No intersections with "
                f"{self.schema_util.flatten_schema_include_parents_fields(df.schema)}. "
        ...
        fields_to_drop = (
            ...
            .find_fields_to_drop(
                flattened_fields=set(
                    SparkSchemaUtility().flatten_schema(df.schema)),
                whitelist_fields=self.whitelist_fields
            )
        )
        log.debug(
            f"Whitelisting {self.whitelist_fields} requires dropping {fields_to_drop}")
        return drop(df, fields_to_drop)

    @staticmethod
    def find_fields_to_drop(
            flattened_fields: Set[str],
            whitelist_fields: Set[str]) -> Set[str]:
        # If the parent is in the whitelist and some of the children are also
        # in the whitelist, only the parent is kept.
        whitelist_fields = filter_only_parents_fields(whitelist_fields)
        fields_to_drop = [
            field for field in flattened_fields if not
            WhitelistProcessor.is_field_or_ancestor_in_whitelist_fields(
                field=field, whitelist_fields=whitelist_fields)
        ]
        return fields_to_drop

    @staticmethod
    def is_field_or_ancestor_in_whitelist_fields(
            field: str, whitelist_fields: Set[str]) -> bool:
    ...
    @classmethod
    def no_fields_to_select(
            cls,
            df: DataFrame,
            whitelist_fields: Set[str]) -> bool:
        flattened_fields = set(
            cls.schema_util.flatten_schema_include_parents_fields(df.schema))
...
def filter_only_parents_fields(fields: Set[str]) -> Set[str]:
    ...
    if len(root_elements) != 0:
        logging.warning(
            f"Root elements found {root_elements}. Be careful!! Child elements will be ignored!")
    return __replace_elements_with_roots(fields, root_elements)


def __replace_elements_with_roots(
        fields: Set[str],
