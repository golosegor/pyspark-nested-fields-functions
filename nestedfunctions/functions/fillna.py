...
# Below function mimics the vanilla pyspark fillna functionality with
# added support for filling nested fields
...
def fillna(
    ...
    if not isinstance(value, (float, int, str, bool, list, dict)):
        raise TypeError(
            f"value argument should be int, float, string, bool, list or dict and is {type(value)}")

    if isinstance(value, dict):
        for col_name, value_to_fill in value.items():
            if __col_name_can_be_filled_with_value(
                    df.schema, col_name, value_to_fill):
        ...
        if isinstance(subset, str):
            ...
            raise TypeError(
                f"subset argument should be list or tuple and is {type(subset)}")
...
def __col_name_can_be_filled_with_value(
        schema: StructType,
        col_name: str,
        value: Any) -> bool:
    ...
    if column_spark_type == value_spark_type:
        ...
        value_array_element_type = value_spark_type.simpleString().replace("array<", "")[
            :-1]
        if value_array_element_type == column_spark_type.simpleString():
            return True
        elif value_array_element_type in ["void", "null"]:
            # value is an empty list and col_name is an array so can fill
            # col_name with value
            return True
        else:
            return False
...
class FillNaProcessor(AnyLevelCoreProcessor):
    ...
    def apply_terminal_operation_on_root_level(
        ...
        field_type: AtomicType = SparkSchemaUtility.schema_for_field(
            df.schema, column_name)
        if SparkSchemaUtility.is_array(df.schema, column_name):
            if isinstance(self.value, list):
                # Fill the array when it's null
                return df.withColumn(
                    column_name, F.when(
                        F.col(column_name).isNull(), F.lit(
                ...
                return df.withColumn(
                    ...
                    F.transform(
                        F.col(column_name),
                        lambda d: self.__coalesce(d)))
        else:
            return df.withColumn(
                column_name, self.__coalesce(
                    F.col(column_name)))

    def apply_terminal_operation_on_structure(
            self,
            schema: StructType,
            column: Column,
            column_name: str,
        ...
        if is_array:
            if isinstance(self.value, list):
                return column.withField(
                    f"`{column_name}`", F.when(
                        column.getField(column_name).isNull(), F.lit(
                            self.value)).otherwise(
                        column.getField(column_name)))
            else:
                return column.withField(
                    f"`{column_name}`",
                    F.transform(
