...
class AnyLevelCoreProcessor(CoreProcessor):
    def __init__(self, column_to_process: str):
        self.column_to_process = validate_field_name_or_throw(
            column_to_process)

    def process(self, df: DataFrame) -> DataFrame:
        if not SparkSchemaUtility.does_column_exist(
                df.schema, self.column_to_process):
            log.error(
                f'Column `{self.column_to_process}` does not exist. This column will not be processed')
            return df
    ...
    def apply_terminal_operation_on_root_level(
        ...
        raise NotImplementedError("Not implemented. Must be overridden")
    ...
    def _process_field_recursive(self,
        ...
        if is_array:
            ...
            column = F.col(current_column_name) if self.__is_root_level_transformation(
                current_column) else current_column.getField(current_column_name)
            return F.transform(
                column,
                lambda d: self._process_field_with(
                    schema=schema,
            ...
            # non-array case. Calculating next column (based on 'root/non-root'
            # level)
            ...
            # If the column is a struct this behavior is undesired so we first
            # fill the column with a struct that has NULL values for all of its
            # fields.
            col = self.__fill_null_struct_with_null_fields(
                col, previous, schema)
    ...
    def __fill_null_struct_with_null_fields(
        ...
        if not SparkSchemaUtility.is_struct(dataframe_schema, column_name):
            raise TypeError("column argument should be a struct !")

        column_schema = SparkSchemaUtility.schema_for_field(
            dataframe_schema, column_name)
