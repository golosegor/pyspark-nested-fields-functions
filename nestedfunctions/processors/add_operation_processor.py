...
class AddOperationProcessor(AnyLevelCoreProcessor):
    ...
    def __init__(self, column_to_process: str, new_column_name: str):
        ...
        (parent_column_to_process, _) = schema_utility.parent_child_elements(
            column=column_to_process, raise_exception_if_no_parent=False
        )
        (parent_new_column, child_new_column) = schema_utility.parent_child_elements(
            column=new_column_name, raise_exception_if_no_parent=False
        )
        if parent_new_column != parent_column_to_process:
            raise ValueError(
                f"Columns `{column_to_process}` and `{new_column_name}` must be at the same level !"
            )
        if not parent_new_column:
            raise ValueError(
                f"new_column_name is {new_column_name} but root level transformation is not supported on AddOperationProcessor"
            )
    ...
    def transform_primitive(
        self, primitive_value: Column, fieldType: AtomicType
    ) -> Column:
    ...
    def apply_terminal_operation_on_root_level(
        ...
        raise Exception(
            "Root level transformation is not supported on AddOperationProcessor"
        )

    def apply_terminal_operation_on_structure(
        self, schema: StructType, column: Column, column_name: str, previous: str
    ) -> Column:
        ...
        if is_array:
            return column.withField(
                f"`{self.new_field_name}`",
                F.transform(
                    column.getField(column_name),
                    lambda d: self.transform_primitive(d, field_type),
                ),
            )
        else:
            return column.withField(
                f"`{self.new_field_name}`",
                self.transform_primitive(column.getField(column_name), field_type),
            )
