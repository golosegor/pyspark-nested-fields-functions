...
class RedactTest(SparkBaseTest):
    ...
    def test_redact_root_level(self):
        ...
        self.assertEqual(
            [SPARK_TYPE_TO_REDACT_VALUE[StringType()]], parse_data(processed))

    def test_redact_nested_structure(self):
        def parse_data(df_to_extract: DataFrame) -> List[str]:
            return [d[0] for d in df_to_extract.select(
                "customDimensions.Metabolics Conditions").collect()]
        ...
        self.assertEqual([expected_value, expected_value],
                         flatten(parse_data(processed)))

    def test_redact_nested_structure_null_value_preserved(self):
        def parse_data(df_to_extract: DataFrame) -> List[str]:
            return [d[0] for d in df_to_extract.select(
                "customDimensions.null value in nested").collect()]
        ...
        expected_value = SPARK_TYPE_TO_REDACT_VALUE[primitive_value_type]
        self.assertEqual([None, expected_value],
                         flatten(parse_data(processed)))
    ...
    def test_redact_throws_exception_if_field_is_not_primitive(self):
        ...
        with pytest.raises(Exception) as excinfo:
            ...
            self.assertIn("Only primitive types could be redacted",
                          str(excinfo.value))
    ...
    def parse_df(self) -> DataFrame:
        df = parse_df_sample(self.spark,
                             os.path.join(FIXTURES_DIR, "redact_sample.json"))
        return df
    ...
    def test_default_redacted_values_has_no_casting_issues(self):
        ...
        def parse_data(df_to_parse: DataFrame) -> Any:
            return [d[0] for d in df_to_parse.select(
                column_to_experiment_with).collect()][0]
        ...
        for spark_atomic_type in SPARK_TYPE_TO_REDACT_VALUE.keys():
            try:
                ...
                processed = df.withColumn(
                    column_to_experiment_with,
                    column_name_with_dedicated_field_type(spark_atomic_type))
                self.assertEqual(parse_data(processed),
                                 SPARK_TYPE_TO_REDACT_VALUE[spark_atomic_type])
                self.assertEqual(
                    spark_atomic_type,
                    processed.schema[column_to_experiment_with].dataType,
                    "Failed to cast type")
                log.info(f"Type `{spark_atomic_type}` successfully processed")
            except Exception as e:
                log.error(
                    f"An exception occurred during processing type {spark_atomic_type}", e)
