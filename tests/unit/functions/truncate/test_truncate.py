...
class TruncateTest(SparkBaseTest):
    ...
    def test_primitive_array(self):
        self.__execute_array_flow(
            "address.zipCodes", 2, expected_result=["65", "76", "87"])

    def test_primitive_array_negative_supported(self):
        self.__execute_array_flow("address.zipCodes", -
                                  2, expected_result=["6543", "76543", "876543"])

    def __execute_array_flow(
            self,
            field: str,
            size: int,
    ...
    def test_root_level_reversed(self):
        self.__execute_test_root_level(
            "creditCard", -2, expected_result="1234")

    def __execute_test_root_level(
            self,
            field: str,
            size: int,
            expected_result: str):
    ...
    def test_truncate_processor_factory_throws_exception_if_field_is_invalid(
            self):
        df = parse_df_sample(
            self.spark,
            os.path.join(
