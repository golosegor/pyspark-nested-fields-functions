...
class HashingTest(SparkBaseTest):
    ...
    def test_root_level_hash(self):
        ...
        df = parse_df_sample(self.spark,
                             os.path.join(FIXTURES_DIR, "root_hash.json"))
    ...
    def test_root_level_primitive_array(self):
        ...
        df = parse_df_sample(
            ...
            os.path.join(
                FIXTURES_DIR,
                "root_level_primitive_array.json"))
        self.assertEqual(parse_data(
            df), ["email@veryhotmail.com", "name@geemail.com"])
        hashed = hash_field(df, field="emails")
        manual = df.withColumn("emails", F.transform(
            "emails", lambda d: F.sha2(d, 256)))
        self.assertEqual([hash_udf("email@veryhotmail.com"),
                         hash_udf("name@geemail.com")], parse_data(hashed))
        self.assertEqual(hashed.collect(), manual.collect())
    ...
    def test_one_level_nested_primitive_array(self):
        ...
        df = parse_df_sample(
            ...
            os.path.join(
                FIXTURES_DIR,
                "one_level_primitive_array.json"))
        self.assertEqual(parse_data(
            df), ["email@veryhotmail.com", "name@geemail.com"])
        manual = df.withColumn(
            "emails", F.col("emails").withField(
                "unverified", F.transform(
                    "emails.unverified", lambda d: F.sha2(
                        d, 256))))
        hashed = hash_field(df, field="emails.unverified")
        self.assertEqual(parse_data(hashed), [hash_udf(
            "email@veryhotmail.com"), hash_udf("name@geemail.com")])
        self.assertEqual(hashed.collect(), manual.collect())
    ...
    def test_three_level_nested_array(self):
        ...
        df = parse_df_sample(
            ...
            os.path.join(
                FIXTURES_DIR,
                "three_level_nested_array.json"))
        self.assertEqual(parse_data(df), ["my-id", "my-id2"])
        manual = df.withColumn(
            "data", F.col("data").withField(
                "city", F.col("data.city").withField(
                    "addresses", F.transform(
                        "data.city.addresses", lambda c: c.withField(
                            "id", F.sha2(
                                c.getField("id"), 256))))))
    ...
    def test_three_level_nested_field(self):
        ...
        df = parse_df_sample(
            ...
            os.path.join(
                FIXTURES_DIR,
                "three_level_nested_field.json"))
        self.assertEqual(parse_data(df), "data-to-hash")
        manual = df.withColumn(
            "data", F.col("data").withField(
                "city", F.col("data.city") .withField(
                    "address", F.col("data.city.address").withField(
                        "id", F.sha2(
    ...
    def test_two_level_nested_field(self):
        ...
        df = parse_df_sample(
            ...
            os.path.join(
                FIXTURES_DIR,
                "two_level_nested_field.json"))
        self.assertEqual(parse_data(df), "data-to-hash")
        hashed = hash_field(df, field="data.address.id")
        self.assertEqual(parse_data(hashed), hash_udf("data-to-hash"))
        manual_script = df.withColumn(
            "data", F.col("data").withField(
                "address", F.col("data.address").withField(
                    "id", F.sha2(
    ...
    def test_double_array(self):
        ...
        df = parse_df_sample(self.spark,
                             os.path.join(FIXTURES_DIR, "double_array.json"))
        self.assertEqual(parse_data(df), ["my-id", "my-id2"])
        ...
        manual = df.withColumn(
            ...
            F.transform(
                F.col("data"),
                lambda e: e.withField(
                    "addresses",
                    F.transform(
                        e.getField("addresses"),
                        lambda c: c.withField(
                            "id",
                            F.sha2(
                                c.getField("id"),
    ...
    def test_hashing_array_with_predicated_on_root_level_works_fine(self):
        df = parse_df_sample(
            ...
            os.path.join(
                FIXTURES_DIR,
                "array_on_root_level_transformation.json"))
        ...
        hashed = hash_field_with_predicate(
            df, "customDimensions.index", "value", "SUPER_SECRET_VALUE")
        self.assertEqual(parse_data(hashed), {
    ...
    def test_hashing_array_with_predicated_on_nested_level_works_fine(self):
        def parse_data(df: DataFrame) -> Dict[str, str]:
            custom_dimensions = df.select(
                "hits.custom dimensions").collect()[0][0]
            return {r['index']: r['value'] for r in custom_dimensions[0]}
        ...
        df = parse_df_sample(
            ...
            os.path.join(
                FIXTURES_DIR,
                "array_on_nested_level_transformation.json"))
    ...
    def test_four_level_primitive_array(self):
        def parse_data(df: DataFrame) -> str:
            return df.collect()[
                0][0][0][0][0][0]["parcels"][0]["trackingData"]["trackingId"]

        df = parse_df_sample(
            self.spark,
            os.path.join(
                FIXTURES_DIR,
                "four_level_nested_array.json"))
        ...
        hashed = hash_field(
            df, field="results.shippingInfo.deliveries.parcels.trackingData.trackingId")
