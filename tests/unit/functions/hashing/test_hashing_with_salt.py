...
class HashingWithSaltTest(SparkBaseTest):
    ...
    def test_root_level_hash_with_salt(self):
        ...
        df = parse_df_sample(self.spark, os.path.join(
            FIXTURES_DIR, "root_hash.json"))
        self.assertEqual(parse_data(df), "my-key-to-hash")
        hashed = hash_field_with_salt(df, "id")
        self.assertEqual(
            hash_udf(f"{DEFAULT_SALT_VALUE}{DEFAULT_SALT_SEPARATOR}my-key-to-hash"),
            parse_data(hashed))
    ...
    def test_root_level_hash_with_salt_empty_field(self):
        ...
        self.assertEqual(
            hash_udf(f"{DEFAULT_SALT_VALUE}{DEFAULT_SALT_SEPARATOR}"),
            parse_data(hashed))

    def test_hash_with_salt_processor_throws_exception_if_field_is_invalid(
            self):
        df = parse_df_sample(self.spark, os.path.join(
            FIXTURES_DIR, "root_hash.json"))
