...
class DropTest(SparkBaseTest):
                                    ...
                                    StructField('mailboxes',
                                                ArrayType(
                                                    StructType(
                                                        [
                                                            StructField(
                                                                "box_number", IntegerType(), True),
                                                            StructField(
                                                                "suffix", StringType(), True)
                                                        ]
                                                    ), True), True),
                                    StructField(
                                        'zipCode', StringType(), True),
                                ]
                            ), True), True),
        ]
    )
    ...
    def test_only_drop_single_field(self):
        ...
        json_str_without_postal_code = (self.COMPLEX_SCHEMA.json() .replace(
            '{"metadata":{},"name":"postalCode","nullable":true,"type":"string"},', ''))
        expected_schema = StructType.fromJson(
            json.loads(json_str_without_postal_code))
        dropped_df = drop(
            empty_df, [
                "addresses.postalCode", "addresses.postalCode", "non-existing"])
        self.assertEqual(dropped_df.schema, expected_schema)
    ...
    def test_drop_all_fields(self):
        ...
        self.assertEqual(
            sorted(["creditCard", "addresses"]), sorted(fields_to_drop))
                                    ...
                                    StructField('pages',
                                                ArrayType(
                                                    StructType(
                                                        [
                                                            StructField(
                                                                'pagePathLevel1', StringType(), True),
                                                        ]
                                                    ), True), True),
                                ]
                            ), True)
    ...
    def test_drop_parent_iso_all_childs(self):
        ...
        self.assertEqual(
            {"creditCard", "addresses.mailboxes"}, set(fields_to_drop))

        empty_df = self.spark.createDataFrame([], self.COMPLEX_SCHEMA)
        json_str_without_creditcard_and_mailboxes = (
            empty_df.drop("creditCard").schema.json() .replace(
                '{"metadata":{},"name":"mailboxes","nullable":true,"type":{"containsNull":true,"elementType":{"fields":[{"metadata":{},"name":"box_number","nullable":true,"type":"integer"},{"metadata":{},"name":"suffix","nullable":true,"type":"string"}],"type":"struct"},"type":"array"}},',
                ''))
        expected_schema = StructType.fromJson(
            json.loads(json_str_without_creditcard_and_mailboxes))
        dropped_df = drop(empty_df,
                          ["creditCard",
                           "addresses.mailboxes.box_number",
                           "addresses.mailboxes.suffix"])
    ...
    def test_drop_parent_and_field_at_same_level(self):
        fields_to_drop = DropProcessor.consolidate_fields_to_drop(
            ...
            fields_to_drop=["addresses.zipCode",
                            ...
                            "addresses.mailboxes.suffix"])
        self.assertEqual(
            {"addresses.zipCode", "addresses.mailboxes"}, set(fields_to_drop))

        empty_df = self.spark.createDataFrame([], self.COMPLEX_SCHEMA)
        json_str_without_zipcode_and_mailboxes = (self.COMPLEX_SCHEMA.json() .replace(',{"metadata":{},"name":"zipCode","nullable":true,"type":"string"}', '') .replace(
            ',{"metadata":{},"name":"mailboxes","nullable":true,"type":{"containsNull":true,"elementType":{"fields":[{"metadata":{},"name":"box_number","nullable":true,"type":"integer"},{"metadata":{},"name":"suffix","nullable":true,"type":"string"}],"type":"struct"},"type":"array"}}', ''))
        expected_schema = StructType.fromJson(
            json.loads(json_str_without_zipcode_and_mailboxes))
        dropped_df = drop(empty_df,
                          ["addresses.zipCode",
                           "addresses.mailboxes.box_number",
                           "addresses.mailboxes.suffix"])
    ...
    def test_drop_while_walking_tree(self):
        ...
        self.assertEqual({"addresses.zipCode",
                          "addresses.mailboxes.suffix"},
                         set(fields_to_drop))

        empty_df = self.spark.createDataFrame([], self.COMPLEX_SCHEMA)
        json_str_without_zipcode_and_mailboxes_suffix = (
            self.COMPLEX_SCHEMA.json() .replace(
                ',{"metadata":{},"name":"zipCode","nullable":true,"type":"string"}',
                '') .replace(
                ',{"metadata":{},"name":"suffix","nullable":true,"type":"string"}',
                ''))
        expected_schema = StructType.fromJson(json.loads(
            json_str_without_zipcode_and_mailboxes_suffix))
        dropped_df = drop(
            empty_df, ["addresses.zipCode", "addresses.mailboxes.suffix"])
        self.assertEqual(dropped_df.schema, expected_schema)
    ...
    def test_drop_3_levels_deep(self):
        ...
        empty_df = self.spark.createDataFrame([], self.COMPLEX_SCHEMA)
        json_str_without_mailboxes_suffix = (self.COMPLEX_SCHEMA.json() .replace(
            ',{"metadata":{},"name":"suffix","nullable":true,"type":"string"}', ''))
        expected_schema = StructType.fromJson(
            json.loads(json_str_without_mailboxes_suffix))
