...
class FillNaTest(SparkBaseTest):
    ...
    def test_fillna_root(self):
        ...
        df = parse_df_sample(self.spark,
                             os.path.join(FIXTURES_DIR, "fillna_sample.json"))
    ...
    def test_fillna_root_array_is_null(self):
        ...
        test_df = df.withColumn(
            "test_array", F.lit(None).cast("array<string>"))
    ...
    def test_fillna_null_inside_root_array(self):
        ...
        df = parse_df_sample(self.spark,
                             os.path.join(FIXTURES_DIR, "fillna_sample.json"))
        self.assertEqual(parse_supported_systems(df),
                         ['SAP_S4HANA_CLOUD', None])
        ...
        self.assertEqual(parse_supported_systems(processed),
                         ['SAP_S4HANA_CLOUD', "UNKNOWN"])
    ...
    def test_fillna_boolean_field_within_struct_within_nested_arrays(self):
        ...
        df = parse_df_sample(self.spark,
                             os.path.join(FIXTURES_DIR, "fillna_sample.json"))
        self.assertEqual(parse_item_store_is_on_stock(
            df, item_index=0, store_index=0), True)
        self.assertEqual(parse_item_store_is_on_stock(
            df, item_index=0, store_index=1), None)
        self.assertEqual(parse_item_store_is_on_stock(
            df, item_index=1, store_index=0), True)
        self.assertEqual(parse_item_store_is_on_stock(
            df, item_index=1, store_index=1), None)
        ...
        self.assertEqual(parse_item_store_is_on_stock(
            processed, item_index=0, store_index=0), True)
        self.assertEqual(parse_item_store_is_on_stock(
            processed, item_index=0, store_index=1), False)
        self.assertEqual(parse_item_store_is_on_stock(
            processed, item_index=1, store_index=0), True)
        self.assertEqual(parse_item_store_is_on_stock(
            processed, item_index=1, store_index=1), False)
    ...
    def test_fillna_integer_field_within_struct_within_nested_arrays(self):
        ...
        df = parse_df_sample(self.spark,
                             os.path.join(FIXTURES_DIR, "fillna_sample.json"))
        self.assertEqual(parse_item_store_available_quantity(
            df, item_index=0, store_index=0), 1)
        self.assertEqual(parse_item_store_available_quantity(
            df, item_index=0, store_index=1), None)
        self.assertEqual(parse_item_store_available_quantity(
            df, item_index=1, store_index=0), 2)
        self.assertEqual(parse_item_store_available_quantity(
            df, item_index=1, store_index=1), None)

        processed = fillna(
            df, subset="payload.lineItems.availability.stores.availableQuantity", value=0)
        self.assertEqual(parse_item_store_available_quantity(
            processed, item_index=0, store_index=0), 1)
        self.assertEqual(parse_item_store_available_quantity(
            processed, item_index=0, store_index=1), 0)
        self.assertEqual(parse_item_store_available_quantity(
            processed, item_index=1, store_index=0), 2)
        self.assertEqual(parse_item_store_available_quantity(
            processed, item_index=1, store_index=1), 0)
    ...
    def test_fillna_null_element_within_array_of_arrays(self):
        ...
        df = parse_df_sample(self.spark,
                             os.path.join(FIXTURES_DIR, "fillna_sample.json"))
        self.assertEqual(parse_comments_for_line_item(df, item_index=0), [
                         'Manually triggered stock check', None])
        ...
        processed = fillna(
            df, value={"payload.lineItems.comments": "Empty comment"})
        self.assertEqual(parse_comments_for_line_item(processed, item_index=0), [
                         'Manually triggered stock check', "Empty comment"])
        self.assertEqual(parse_comments_for_line_item(
            processed, item_index=1), None)

        # To fill array within array which is null specify list of strings as value
        try:
            processed = fillna(df, value={"payload.lineItems.comments": [
                               "Automatically triggered stock check"]})
        except:
            # In Spark versions < 3.4.0 get below error when try F.lit(some_list)
            # org.apache.spark.SparkRuntimeException: The feature is not supported: literal for '[Automatically triggered stock check]'
            # of class java.util.ArrayList.
        ...
        self.assertEqual(parse_comments_for_line_item(processed, item_index=0), [
                         'Manually triggered stock check', None])
        self.assertEqual(parse_comments_for_line_item(processed, item_index=1), [
                         "Automatically triggered stock check"])
    ...
    def test_fillna_multiple_root_columns(self):
        ...
        # Check if the null elements within an array are being filled correctly
        processed = fillna(
            df, value={"supportedSystems": "UNKNOWN", "eventVersion": "1.0"})
        self.assertEqual(parse_event_version(processed), "1.0")
        self.assertEqual(parse_supported_systems(processed),
                         ['SAP_S4HANA_CLOUD', "UNKNOWN"])
    ...
    def test_fillna_ancestor_struct_is_null(self):
        ...
        path = os.path.join(
            FIXTURES_DIR, "fillna_ancestor_struct_missing.json")
        ...
        for i in range(2):
            ...
            self.assertEqual(
                processed_line_items_dict[i]["availability"]["availableQuantity"], 0)
            ...
            self.assertEqual(
                processed_line_items_dict[i]["availability"]["availableQuantityForSize"]["availableQuantityS"], 0)
            ...
            self.assertEqual(
                processed_line_items_dict[i]["availability"]["availableQuantityForColor"], None)
