...
class StringContactTest(SparkBaseTest):
    ...
    def test_str_concat_processor_factory_throws_exception_if_field_is_invalid(
            self):
        df = parse_df_sample(
            self.spark,
            os.path.join(
    ...
    def test_str_concat_processor_factory_not_throws_exception_if_field_is_invalid(
            self):
        df = parse_df_sample(
            self.spark,
            os.path.join(
